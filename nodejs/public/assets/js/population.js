...
function drawRegionsMap() {
	$.ajax({
		...
		error: function () {
			...
			nodata.innerHTML = '<span class="pe-7s-junk" style="font-size: 20px;"></span> An error occurred while contacting server...';
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.length == 0) {
				var nodata = document.createElement('div');
				nodata.className = 'alert alert-warning';
				nodata.innerHTML = '<span class="pe-7s-junk" style="font-size: 20px;"></span> No data to display';
				document.getElementById('chart1').appendChild(nodata);
				return;
			}
